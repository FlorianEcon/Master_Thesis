...
/*
	...
	Note:
		All Tables in this file are only raw version which are later adjusted in LaTeX
*/
...
****************************************************************************************************
// 1. Summary Statistics and Preliminaries
****************************************************************************************************
	**********************************************
	// i. Preliminary Data Work//
	**********************************************
		// Some additional Data Work and Cleaning needed for estimations and summaries
		// Drop counties without any Weather Data
		gen rep = 1 if prcp != .
		// Identify counties counties without any rain data
		bys geographicareaname: egen sumrep = sum(rep)
		// This affects 63 counties (or 4.2%)

		// drop these counties
		drop if sumrep == 0
		drop sumrep rep

		// remove counties, where due to missings in crimes, weather or mobility, only missings are left (10 counties)
...
****************************************************************************************************
	...
	**********************************************
		// Note: For the xtivreg command vce(r) equals clustering on the paneld id variable. However, vce(cluster) is not allowed in xtivreg and produces an error, so the code shows only vce(r) while still producing clustered SE
		// In the following regression we analyze the effet of both opporutnity variables on total crimes as well as on the two aggregates property and violet crimes.
		*******************
		// a) Total Crimes
		*******************
			// FIXED EFFECTS	
			...
			// Two tables for each opportunity variable
			// Population at Home
		...
		*******************
		// b) Property Crimes
		*******************
		// Note: Starting from here, we only use the Poisson FE - IV estimator with Month FE. 
			// POISSON
			// IV & Month
		...
		*******************	
		...
		// b) and c) are used in later tables accompanied with their respective subcrimes
					
	  
	**********************************************
	// iii. Crime Heterogeneity	- Table OC2	//
	**********************************************
	// The following estimations allow for heterogeneity between different offenses. Tables are then sorted by their respective motives (property or violent)
	// Estimations are done using the Poisson-FE IV estimtor and include month fixed effects. SE are clustered on the county level.
		*******************
		// a) Property Crimes
		*******************
			// Robbery
			...
			// Table OC2a
			// Population at Home
			esttab OC1b_P_PIVM_Prop OC2a_P_Rob OC2a_P_Lar OC2a_P_Bur OC2a_P_MVT using "C:\Users\Flori\OneDrive\Desktop\Uni\Emma\Dataset/Tables/OC2aHome.tex", keep(PopulationAtHome) title("Opportunity Home for Property Offenses") coeflabels(PopulationAtHome "\% Pop at Home") mtitles("Property Crimes" "Robbery" "Larcency" "Burglary" "MVT")  scalars("N_g Cluster") replace
			
			// Trips p.P.
			esttab  OC1b_T_PIVM_Prop OC2a_T_Rob OC2a_T_Lar OC2a_T_Bur OC2a_T_MVT using "C:\Users\Flori\OneDrive\Desktop\Uni\Emma\Dataset/Tables/OC2aTrip.tex", keep(trips_pP) title("Opportunity Trips on Property Offenses") coeflabels(trips_pP "Trips p.P.") mtitles("Property Crimes" "Robbery" "Larcency" "Burglary" "MVT") scalars("N_g Cluster") replace
		...
		*******************
			...
			// Table OC2b
			// Population at Home
			esttab OC1c_P_PIVM_Vio OC2b_P_AA OC2b_P_Mur OC2b_P_Rape using "C:\Users\Flori\OneDrive\Desktop\Uni\Emma\Dataset/Tables/OC2bHome.tex", keep(PopulationAtHome) title("Opportunity Home for Property Offenses") coeflabels(PopulationAtHome "\% Pop at Home") mtitles("Violent Crimes" "AA" "Murder" "Rape") scalars("N_g Cluster") replace
			
			// Trips p.P.
			esttab OC1c_T_PIVM_Vio OC2b_T_AA OC2b_T_Mur OC2b_T_Rape using "C:\Users\Flori\OneDrive\Desktop\Uni\Emma\Dataset/Tables/OC2bTrip.tex", keep(trips_pP) title("Opportunity Trips on Property Offenses") coeflabels(trips_pP "Trips p.P.") mtitles("Violent Crimes" "AA" "Murder" "Rape") scalars("N_g Cluster") replace
		
			
	**********************************************
	// iv. Daytime Heterogeneity	- Table OC3	//
	**********************************************
	// The following estimations allow for heterogeneity between different offenses and the time they where commited. Daytime is defined as 5am-5pm.
	// Tables are then sorted by their respective motives (property or violent)
	// Estimations are done using the Poisson-FE IV estimtor and include month fixed effects. SE are clustered on the county level.		
		*******************
		// a) Aggregate Crimes
		*******************
	...
	**********************************************
	// The following estimations allow for heterogeneity between different offenses and the location they where commited. Locations are defined by the codes included in NIBRS.
	// Tables are then sorted by their respective motives (property or violent)
	// Estimations are done using the Poisson-FE IV estimtor and include month fixed effects. SE are clustered on the county level.		
		*******************
		// a) Aggregate Crimes
