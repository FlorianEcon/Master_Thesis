'''
    Created: 20.1.2021
    Name: FBI_UCR_API_Crawler
    Author: Florian Fickler
    Goal:
            Simple crawler to access the API set up by the FBI
            Create summary statistics for each police Agency in the USA in 2019
    To-Do for it to run:
...
# CSV Writer
########
# Slightly change Data structure
def data_month():
    # Initialize month dictionary for 2019
    month_dict = {}

    for j in range(len(ori_codes)):
        # Extract ori code
        l1 = crime_data_monthly.get(ori_codes[j])

        # Extract monthly data for each ori code
        month = list(l1.keys())

        # loop through month (some differ from 12)
        for i in range(len(month)):
            # Extract monthly data
            l2 = l1.get(month[i])

            # Insert the ori-code and the month number into the monthly data
            l2.update({"month": month[i],
                       "ori": ori_codes[j]})

            # Replace the month dictionary with the new data containing the month number and the ORI for each entry
            month_dict[ori_codes[j], month[i]] = l2

    return month_dict


# Alter the downloaded data, so that ORI-codes and month numbers are included within each monthly entry
month_data = data_month()


# Define fields - Headers for the CSV
# Using an Agency, that has all entries
fields = list(month_data['AK0010100', '1'].keys())

# Write data rowhise into a CSV
with open("month_data.csv", "w", newline='') as f:
    w = csv.DictWriter(f, fields)
    w.writeheader()
    for k in sorted(month_data):
        w.writerow(month_data[k])
